...
final class LocalLLMService {
    ...
    /// Deletes the downloaded model weights from the Hugging Face cache,
    /// freeing disk space. Safe to call even if nothing was downloaded, or
    /// while the Simulator's guard makes `isCachedOnDisk` always report
    /// `false`.
    func deleteDownload() {
        #if !targetEnvironment(simulator)
        unload()
        guard let repo = HuggingFace.Repo.ID(rawValue: modelChoice.repoID) else { return }
        let directory = HuggingFace.HubCache.default.repoDirectory(repo: repo, kind: .model)
